...
configuration-screen-default-options-action-description = 特定のニーズがない場合は、バニラパックの最初の設定としてお勧めします。{ -app-name } はほとんどの場合、"正常な動作"をします。
configuration-screen-custom-options-action = カスタマイズした設定を使用する
configuration-screen-custom-options-action-description = { -app-name }の動作方法を変更する場合や、ModのサポートやZIPの保護などの機能を使用したい場合は、これを選択してください。
