...
async fn read_previous_zip_contents(
	...
	let end_of_central_directory_offset = EndOfCentralDirectory::locate(&mut previous_zip)
		.await?
		.ok_or(PreviousZipParseError::Invalid(
			"EOCD not found, is this a valid ZIP file generated by PackSquash?"
		))?;

	// Seek to the beginning of the end of central directory record, skipping the signature
	// we know is there due to the `locate` method contract
	previous_zip
		.seek(SeekFrom::Start(
			end_of_central_directory_offset + EndOfCentralDirectory::SIGNATURE.len() as u64
		))
		.await?;
	...
	if central_directory_offset == 0xFFFFFFFF {
		...
		if buffer[..4] == EndOfCentralDirectory::ZIP64_LOCATOR_SIGNATURE {
			...
			if buffer[..4] != EndOfCentralDirectory::ZIP64_SIGNATURE {
	...
	while {
		...
		buffer[..4] == CentralDirectoryHeader::SIGNATURE
	} {
		...
		if buffer[..4] != LocalFileHeader::SIGNATURE {
			return Err(PreviousZipParseError::Invalid(
