...
pub(super) fn get_dbus_machine_id() -> Option<SystemId> {
	...
	.ok()
	.map(|id| SystemId::new(id.to_le_bytes(), false))
}
...
pub(super) fn get_host_id() -> Option<SystemId> {
	...
	#[allow(unsafe_code)]
	Some(SystemId::new(unsafe { gethostid() }.to_le_bytes(), true))
}
...
pub(super) fn get_machine_id() -> Option<SystemId> {
	...
	Some(SystemId::new(
		Uuid::try_parse(&machine_guid).ok()?.into_bytes(),
		false
	))
}
...
pub(super) fn get_system_root_volume_id() -> Option<SystemId> {
	...
	fs::metadata(r"\\?\GLOBALROOT\SystemRoot")
		...
		.map(|volume_serial_number| SystemId::new(volume_serial_number.to_le_bytes(), false))
}
