//! Contains the configuration options for processing a pack.
...
#[repr(transparent)]
pub struct ExistingDirectoryPath<'data>(#[serde(borrow)] Cow<'data, Path>);
...
#[serde(try_from = "Cow<'data, Path>")]
#[repr(transparent)]
pub struct MaybeNonexistentFilePath<'data>(#[serde(borrow)] Cow<'data, Path>);
...
#[derive(Deserialize, Serialize, EnumSetType, Display)]
#[serde(rename_all = "snake_case")]
...
file_options_default_impl!(ShaderFileOptions => ShaderFileOptions {
	source_transformation_strategy: ShaderSourceTransformationStrategy::DEFAULT,
	is_top_level_shader: None
});
