...
fn main() {
	let parameters = App::new("PackSquash")
		...
		.args_from_usage("
			...
			[Do not ignore system and hidden files] -a --do-not-ignore-system-and-hidden-files 'If specified, filenames which start with a dot or are likely generated by a software system (Windows, VCS...) won't be skipped without a message'
			[Include files used by modifications] -m --include-mod-files 'If specified, resource pack files ignored by vanilla Minecraft clients, but used by modified clients, will be included and processed. Currently, setting this option enables processing and including OptiFine property files in the output ZIP'
...
fn run(parameters: ArgMatches) -> Result<(), Box<dyn Error>> {
	...
	let process_mod_files = parameters.is_present("Include files used by modifications");
	let threads = cmp::max(
