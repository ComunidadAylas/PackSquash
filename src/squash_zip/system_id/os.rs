...
/// it has the desireable properties of being 128-bit wide, userspace-agnostic,
/// and available in virtually every Linux kernel (at least from 2.2), it
/// changes in each boot, so it's pretty volatile. It relies on sysctl, so it
/// relies on procfs being mounted at /proc.
///
/// Further reading:
/// - https://www.kernel.org/doc/html/latest/admin-guide/sysctl/kernel.html#random
/// - http://0pointer.de/blog/projects/ids.html
#[cfg(any(target_os = "linux", target_os = "android"))]
pub(super) fn get_boot_id() -> Option<(u128, bool, bool)> {
	use std::fs;
	use uuid::Uuid;
...
pub(super) fn get_kernel_host_id() -> Option<(u128, bool, bool)> {
...
#[cfg(any(target_os = "freebsd", target_os = "dragonfly"))]
pub(super) fn get_dmi_product_id() -> Option<(u128, bool, bool)> {
	use std::ffi::{CStr, CString};
	...
	match unsafe {
		...
		33 | 37 => Some((
			...
			.ok()?
			.as_u128(),
			false, // Some hardware vendors are lazy and do not initialize this properly
			false
		)),
...
#[cfg(unix)]
pub(super) fn get_host_id() -> Option<(u128, bool, bool)> {
...
/// official documentation. SMBIOS DMI is used to get this UUID. It may be all zeros if a
/// product UUID is not available, however.
///
/// Further reading:
/// - https://docs.microsoft.com/en-us/windows/win32/cimwin32prov/win32-computersystemproduct
#[cfg(windows)]
pub(super) fn get_product_id() -> Option<(u128, bool, bool)> {
