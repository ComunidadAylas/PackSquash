...
* For Java property files (only if the -m flag is used): minification, by removing unneeded whitespace. Like with JSON, performing minification requires parsing the file, so PackSquash will show any validation error.
