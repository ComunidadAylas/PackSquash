> [!IMPORTANT]
> **This document is for PackSquash version v0.4.0**. Different versions may use
...
- `bad_entity_eye_layer_texture_transparency_blending`: all currently known
  Minecraft versions overlay entity layer textures in a way that does not
...
- `ogg_obfuscation_incompatibility`: not all Minecraft versions are compatible
  ...
  only versions from 1.14 to 1.20.5-pre1 reliably support obfuscated files;
